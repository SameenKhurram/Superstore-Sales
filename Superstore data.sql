...
#Unique Country, City and State
select count(distinct country) as Country, count(distinct City) as City, count(distinct State) as State
from store_sales;

#Sub-categories and products in the segments
select distinct Segment as Segments, count(distinct `Sub-Category`) as Sub_Category, count(distinct product_name) as Product_Name
from store_sales
group by segments;

# Shipping mode by orders
...
#Products with profit more than average profit
with ap as  (select  product_name, round(avg(profit),2) as average_profit
from store_sales
group by product_name
order by average_profit desc)
select ss.product_name, round(ss.profit,2) as profit, ap.average_profit
from store_sales ss
join ap ap
on ap.product_name=ss.product_name
where ss.profit>ap.average_profit;

#Profit by month and year
select monthname(order_date) as Order_Month, year(order_date) as Order_Year,profit
from store_sales
...
#effect of shipping rate on delivery time and its relation with quantity
